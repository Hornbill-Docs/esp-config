...
### Guardrail input rules

Guardrails are specialized system prompts that the system integrates into the core instructions of the HAi Assistant to evaluate user messages.

#### Guardrail components

Use the following four elements to structure guardrail configurations.
...
* **Behavioral triggers**: Define specific user intents, actions, or phrases that trigger the guardrail. Granular triggers reduce the likelihood of false positives where the model blocks safe text. Example: "Trigger when a user requests to export raw log files or database connection strings."
* **Execution rules**: Define the action the model takes when a rule is violated. Specify if the agent must block the input, rewrite the output, or provide a standard refusal response. Example: "If triggered, do not answer the request."
