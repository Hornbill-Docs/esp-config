...
* **Agent Efficiency**: The time saved by your agents using generative HAi features that shift routine tasks from humans, reducing manual workloads and routine touch points.
...
You can use plain text prompts to define guardrail rules. The key is to use strong active verbs and clear conditional logic.

##### Your Keywords

The following key words are processed as high-priority commands rather than conversational text.

* **Do not allow**. This acts as a hard filter. The HAi Assistant immediately scans incoming text looking for whatever follows this phrase to flag it as a violation.
* **Trigger**. This acts like an IF statement in programming. It tells the HAi Assistant, "When you see X condition met, change your behavior immediately."
* **Block**. This is the THEN statement. It tells the the HAi Assistant exactly what mechanism to execute once a trigger has been tripped (e.g., halting text generation, wiping the output, or substituting a refusal message).

##### Example plain text rule

> ***Do not allow** any content that is dangerous, offensive, discriminatory, political, or uses inappropriate language for a business environment. **Block** any requests involving malware or malicious code. You must **trigger** a **block** immediately if a user's input or the requested output provides instructions for bypassing multi-factor authentication. **Block** any requests where the answer is likely to contain any of these disallowed topics.`*

#### Using a structured input

When using plain text, the model can suffer from "loss in the middle," meaning your mid-paragraph rules might get diluted or skipped. The structured format maps its attention directly to specific blocks of text, drastically reducing the chances of a user "slipping a prompt past the guardrail."

Use the following four elements to provide a structured guardrail input.

* **Categorized definitions**: Group restricted concepts into explicit XML tags. Large Language Models (LLMs) categorize text effectively when matching against defined buckets. Examples include `<restricted_topics>`, `<pii_data>`, or `<competitor_mentions>`.
* **Behavioral triggers**: Define specific user intents, actions, or phrases that trigger the guardrail. Granular triggers reduce the likelihood of false positives. Example: "Trigger when a user requests to export raw log files or database connection strings."
* **Execution rules**: Define the action to take when a rule is violated. Specify if the HAi Assistant must block the input, rewrite the output, or provide a standard refusal response. Example: "If triggered, do not answer the request."
* **Allowed exceptions**: Define permitted topics to prevent the guardrail from becoming too restrictive. Example: "Allow discussions regarding internal company policies, such as HR rules, but block commentary on external government elections."

##### Example guardrail input rules using components
...
Discriminatory,
Political,
Inappropriate language in a business environment,
Malware or malicious code
</disallowed_content>

<sensitive_corporate_data>
